      ...
      <div className="relative w-full h-[600px] overflow-hidden">
        {featureImageList && featureImageList.length > 0
          ? featureImageList.map((slide, index) => (
              <img
                 src={slide?.image.includes("http") ? slide?.image  : `/featureImages/${slide?.image}`} 
                key={index}
                className={`${
                  index === currentSlide ? "opacity-100" : "opacity-0"
                } absolute top-0 left-0 w-full h-full sm:object-fill object-contain transition-opacity duration-1000`}
              />
            ))
          : null}
        <Button
